  ...
  return (
    <div className="flex min-h-svh">
      {/* Below lg, the sidebar is an off-canvas drawer (fixed, slides in)
          instead of taking up permanent width — a 240px rail plus content
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-30 flex h-svh w-60 shrink-0 flex-col bg-sidebar p-4 text-sidebar-foreground transition-transform duration-200 lg:sticky lg:top-0 lg:translate-x-0 ${
          navOpen ? "translate-x-0" : "-translate-x-full"
        }`}
        style={{ boxShadow: "var(--clay-shadow-sidebar-edge)" }}
      >
        <div className="mb-6 flex items-center justify-between px-2">
          <span className="text-lg font-semibold text-brand-secondary">SWAMS</span>
          <button
            type="button"
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="clay-raised-sm relative z-10 flex items-center justify-between gap-2 rounded-none bg-card px-4 py-3 sm:px-6">
          <div className="flex items-center gap-2">
            ...
            >
              <Avatar src={user?.profile_picture_url} name={displayName} className="size-7" />
