...
correctly handles both the nullable-organization case (NULL never equals
any concrete session value, so it's hidden from tenant-bound sessions) and
the WILDCARD Super Admin case, without a separate `IS NULL` branch or an
unsafe cast of the session var itself to `uuid`.
"""

from django.db import migrations

TABLE = "security_events_securityevent"
...
class Migration(migrations.Migration):
    dependencies = [("security_events", "0001_initial")]
